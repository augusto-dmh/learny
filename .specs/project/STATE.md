...
- **Next cycle: `cited-qa` (TDD Phase 7) — Not started.** Adds the answer-generation port + provider adapter, a cited-answer endpoint, and "not found in source" behavior over the Phase-6 retrieval layer (`RetrieveEvidence` / `POST /api/sources/{id}/retrieve`). This is the first cycle to wire an LLM answer path; the embedding provider deferred in AD-019 may also want its own ADR here.
