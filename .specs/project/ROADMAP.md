...
| `v5-spoiler-safe-retrieval` | F | Position-scoped retrieval (build now, deploy after the retrospective) | Done (PR #70) — section-order bound enforced inside the shared scoped CTE, uniform across the ask/teach/`/retrieve` book arms (AD-348); unmatched anchor and position-read failure fail closed (AD-349); merged 2026-09-08, auto-deploy took the RFC-005 rollout |
...
|---|---|---|---|
| `house-profiles` | Recorded slice (`docs/research/2026-09-07/provider-adapter-architecture.md` §3.2 Flavor A; PR #69 + PR #70 handoff NEXTs) | Curated per-user house profiles: preference row + two-tier chain resolution + learner-visible profile copy + account selector; ADR-0020 follow-up amendment; BYO keys stay deferred | Done (PR #71) — one preference row per user (migration `0029_user_ai_preferences`); chosen profile leads the registry remainder deduped, unset/unknown/stale falls back to the operator default chain; explain/quiz/card seams stay house-routed; per-answer budget stamp debits the serving profile's catalog; learner-visible copy + AccountPanel selector; ADR-0020 amendment "End-User Choice Among House Profiles" ratified; merged 2026-09-14 |
