...
| WSC-05 | Dock lists this book's conversations, newest first, title + turn count; resume in place | `frontend/tests/reader-panel.test.tsx:204, 227, 241, 249, 265, 281` | ✅ |
| WSC-06 | Rename persists + shows without reload; delete removes and turns become unretrievable; empty state; ask and teach in one list | `conversation-dock.test.tsx:126,156,179,206,226`; `test_web_conversations.py:954` (`test_delete_leaves_no_turn_or_citation_row_behind` — counts rows in `conversation_turns` and `conversation_turn_citations` directly, with a second conversation kept as a scoping control); `reader-panel.test.tsx:227` | ✅ |
...
| WSC-12 | Bounded default page; named window; 422 outside bounds; no row skipped or duplicated on ties | `test_web_conversations.py:621,647,676,711`; killed by M3, M4 | ✅ |
...
| WSC-14 | 409 on turn-index race and stale teach target; 422 on unresolvable scope — preserved through convergence | `test_application_conversations.py:1735,1755,1590,1613`; `test_web_conversations.py:1338,1355,1436,400` | ✅ |
| WSC-15 | No surviving mutating route loses its limiter | `test_web_conversations.py:2099,2123`; `test_retired_surface.py::test_no_throttle_outlives_the_routes_it_guarded`; killed by M5 | ✅ |
...
behavioral sensors at `test_application_conversations.py:1483,1515`.
...
- `test_scoped_turn_never_cites_evidence_outside_its_scope` (`test_web_conversations.py:1178`) does **not** cover this: it constrains what retrieval *returns*, not what grounding filters out of it.
