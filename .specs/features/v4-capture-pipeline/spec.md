...
| CAP-A10 — Reconcile independence | Highlight-derived cards reconcile on their **own** snapshot via the existing `ReconcileQuizItems` ladder, independent of the linked note anchor's outcome; the current ingestion step order (notes before quiz) is pinned by a test | The two reconcilers have no ordering contract today; making cards depend on anchor outcomes would create one and couple two aggregates. Each carries its own excerpt, so each is self-consistent | n (auto-decided) |
