...
**Status**: In Progress — Phase A ✅ complete (A1 `2eb7811` · A2 `e9428ee` · A3 `ba7b901` · A4 `02e0505` · A5 `b0f9c37` · A6 `e14a814`; offline 825 passed/0 failed, DB-gated reader modules 191 passed, ruff clean). Phase deviations: `NoteRepository.highlights_for_source(user_id, source_id)` (name collision with existing `anchors_for_source`); `word_count` = computed property on `CorpusSectionRecord` (build code untouched); migration keeps `NOT NULL DEFAULT 0` (0009 idiom). Chapter route: `anchor` optional — omitted = resume. Phase B ✅ complete (B1 `81cadee` · B2 `1df7a3d` · B3 `0bb07f9` · B4 `d79b5f8`; frontend 273 passed/0 failed, tsc clean). Phase notes: reader lives in `app/components/chapter-reader.tsx` (`ChapterReader` orchestrator + exported `ChapterFlow`); hook `app/components/use-scroll-position.ts` (house-style location); `section-reader.tsx`/`.test.tsx` deleted, citation-loop test re-pointed at `/chapter`; sticky `data-testid="reader-top-bar"` + `.prose-reading` article (`articleRef`) are the C/D seams. Phase C ✅ complete (C1 `9c37871` · C2 `be5ff30` · C3 `31d6090` · C4 `f4f7263`; frontend 304 passed/0 failed, tsc clean). Phase deviations: new hooks live in `app/components/` (`use-reading-settings.ts`, `use-receding-chrome.ts`) not `app/hooks/`, matching Phase B house style. Phase notes: reader-scoped `--reading-size`/`--reading-leading` vars + `data-appearance` applied on the `.prose-reading` article (now also `bg-background text-foreground` so Paper paints); `ReadingControls` is a controlled component (settings+setters as props, `useTheme` for theme); `toc-panel.tsx` exports `TocPanel` + `ChapterNav`, TOC structure-load failure degrades to a quiet muted note (not `role="alert"`) so it never competes with content alerts; ink-line uses `bg-border`/`bg-primary` token utilities (no globals.css change needed, leak scan green); `ChapterFlow` now fetches the book structure via `TocPanel` on every render (a new network dependency for its tests). Phase D ✅ complete (D1 `d761960` · D2 `1bb18fc` · D3 code `65fb85e`+`2a230e5`, statuses this commit; frontend 323 passed/0 failed, tsc clean, build OK; backend offline 825 passed/0 failed, ruff clean; DB-gated reader modules 145 passed/0 failed). Phase notes: pure `findQuoteOffset` + DOM `paintHighlights` (idempotent unwrap-first, `active`-only, marks only wrap, cross-node slices) live in `app/lib/highlight-paint.ts`; `ChapterReader` fetches `listHighlights` post-auth (non-blocking, failure-tolerant) and passes them to `ChapterFlow`, which groups by anchor and paints per section via the extracted `FlowSection` (paint effect keyed on markdown + the section's stable highlight array so re-renders never repaint); `.reader-highlight` mark styles read `--highlight-yellow` (pinned in `theme-tokens.test.ts`); the capture-popover doc-comment now points at `chapter-reader.tsx`. Backend was gate-only (untouched). ENV NOTE: the 11 `test_web_corpus.py` failures in the full DB-gated run are the documented MinIO-503 source-upload noise (external service down), not reader code; `globals.css:172` still carries a looser "section reader" comment (out of D3 scope — left untouched).
