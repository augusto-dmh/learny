...
- **Phase C (accepted):** (1) AuthForm/AccountPanel restyle kept accessible names — auth-screens tests green unmodified. (2) `(app)/layout.tsx` uses a plain flex column, not `SidebarInset` (would nest `<main>` — pages own their own `<main>`). (3) `SourcesPanel.tsx` left in place, dead except the old ask-link test block — Phase D removes it with the panel rebuilds. Sidebar tree-expansion tests supersede the old inline structure-view tests. LibraryScreen fetches `getIngestion` for failed sources (latest-event message per FE-20). (4) Polling-hook tests inject `getIngestion` via `vi.mock` (undici `Response.json()` deadlocks under fake timers — probed); client URL/parsing already covered by the B3 client tests. (5) Status projection mirrored from `app/application/ingestion.py`: succeeded→ready, failed→failed, queued/running→processing.
- **Phase D (accepted, incl. one SPEC_DEVIATION):** (1) SPEC_DEVIATION in `citations.tsx` — AI Elements `InlineCitation`/`Sources` model *web* sources (hostname via `new URL()`, `target=_blank`), unusable for book anchors; CitationList composes the vendored shadcn Popover instead. FE-16 behavior unchanged and tested. (2) `streaming.ts` wraps fetch (`streamingFetch`): ai@7's HttpChatTransport throws `Error(response.text())` with no status on non-OK start, so non-OK → `StreamRequestError(status)` with `errorMessageFor(status)` message, network → "network", AbortError rethrown untouched (stop() settles); 401 triggers `onRequireAuth`. (3) Teach seeds history via ChatInit `messages` with keyed remount per session. (4) Panels deleted with their replacements: AskPanel, SourcesPanel (D3), TeachPanel (D4); screen tests rebuilt intent-preserving. (5) jsdom streaming tests need ResizeObserver/pointer-capture/scrollIntoView polyfills + real v1 SSE frames; never push `[DONE]` after an `error` part.
