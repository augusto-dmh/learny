...
| Check | Result |
| --- | --- |
| TS `pageAt` ≡ backend `page_at` at boundaries | **ENFORCED.** Was a one-time manual evaluation of twelve cases, which agreed but bound nothing — either side could drift afterwards with both suites green. The cases now live in `contracts/page-boundaries.json` (fourteen, adding a negative quantum and both guards at once) and both suites assert every one: `backend/tests/test_reading_pure.py::test_page_at_matches_every_shared_boundary_case` and `frontend/tests/pages.test.ts::pageAt matches every shared boundary case`. |
