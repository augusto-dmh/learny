...
- Verification vocabulary (root `Makefile` — use these names): `make infra` (starts `db`+`minio`+`redis`), `make test-backend` (= `cd backend && uv run pytest`), `make test-frontend` (= `cd frontend && npm test`), `make lint` (backend `ruff check` + `ruff format --check`, frontend `tsc --noEmit`, architecture boundaries — CI parity; ESLint is deliberately not a gate), `make check` (lint + all tests). DB/golden tests need `make infra` first; `LEARNY_TEST_DATABASE_URL` comes from `.claude/settings.json`.
