...
class AnthropicAnswerAdapter:
    ...
    stays inside this module and an injected fake needs no key/network). The
    buffered path calls ``messages.create`` (``max_tokens`` is far below the SDK's
    non-streaming guard) with no sampling or ``thinking`` params.
    """
