"""T11 gate — GET /api/sources/{id}/structure (integration, live test DB).

Exercises the structure endpoint through FastAPI's ``TestClient`` against a real
Postgres: the owner reads the nested section tree (200 with the exact nested shape
and values — titles, depths, section paths, anchors, and child nesting built from
the flat depth-ordered sections), an unauthenticated caller gets 401, and a
missing source, another user's source, and an owned source with no corpus all
return 404 (A-7). The GET carries no CSRF header and must not be rejected.
...
def _section(
    *, position: int, title: str, depth: int, section_path: tuple[str, ...], anchor: str
) -> CorpusSectionRecord:
